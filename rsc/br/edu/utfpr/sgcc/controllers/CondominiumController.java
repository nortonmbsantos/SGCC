...
//	}
	...
	public ModelAndView acceptEntry(@ModelAttribute CondominiumEntryRequest cRequest) {
		...
		if (condominium.getIdUser() == user.getId()) {
			if (service.accept(cRequest.getId())) {
				...
				modelAndView.addObject("result", new Result("Acesso ao condomínio liberado", "success"));
			} else {
				modelAndView.addObject("result", new Result("Falha ao liberar acesso ao condomínio", "error"));
			}
		} else {
	...
	public ModelAndView refuseEntry(@ModelAttribute CondominiumEntryRequest cRequest) {
		...
		if (condominium.getIdUser() == user.getId()) {
			if (service.refuse(cRequest.getId())) {
				modelAndView.addObject("result", new Result("Acesso ao condomínio recusado", "success"));
			} else {
				modelAndView.addObject("result", new Result("Falha ao recusar acesso ao condomínio", "error"));
			}
		}
	...
	public ModelAndView addCondominiumForm(@ModelAttribute @Valid Condominium condominium, BindingResult result) {
		...
		if (condominium != null) {
			...
			modelAndView.addObject("result", new Result("Condomínio criado com sucesso", "success"));
		} else {
			modelAndView.addObject("result", new Result("Falha ao cadastrar o condomínio", "success"));			
		}
		return modelAndView;
	}
